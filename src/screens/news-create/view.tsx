...
export default function NewsCreateView({ locale }: Props) {
		...
		return (
			languages.find(
				lang =>
					...
					normalizeLocale(selectedLanguageCode)
			) ?? null
		);
		...
		const loadLanguages = async () => {
			...
			try {
				...
				const languages: Language[] =
					data?.data?.languages ?? data?.languages ?? [];
	...
	useEffect(() => {
		...
		const defaultLanguage =
			languages.find(
				lang => normalizeLocale(lang.language_code) === normalizeLocale(locale)
			) ?? languages[0];
	...
	const onSubmit = async (
		...
		try {
			...
			const payload = {
				hashtag_names: values.category_names,
							...
							<FormItem>
								<FormControl>
									<CategoryCombobox
										...
										error={!!form.formState.errors.category_names}
										maxLength={50}
									/>
								</FormControl>
							</FormItem>
